...
The pull-request workflows currently fail before evaluating this change. The pinned TFLint and Terraform Test wrappers configure an expired repository AWS access key, while the pinned Checkov wrapper uses `actions/setup-python@v1` with an unavailable Python 3.11.11 build. The Pre-Commit wrapper also downloads Terraform Docs 0.16.0 from a retired URL. Pull request #93 has the same first three failed workflows, and `meta exec buycycle production` confirms that valid managed AWS access plus the changed-module Terraform and TFLint commands succeed.
...
- Modify `.github/workflows/checkov.yaml`: call the official Checkov v12 action directly by immutable commit, with `soft_fail: true` to preserve the current advisory policy for scan findings. Scan deployable repository-owned Terraform without recursively downloading third-party module trees or following test-fixture modules back into their parent modules. Do not use step-level `continue-on-error`, so action setup and scanner execution errors still fail the job.
- Modify `.github/workflows/pre-commit.yaml`: replace the stale shared wrapper with current checkout, Python, and Terraform setup actions; install the checksum-verified Terraform Docs 0.22.0 release; and run the repository hooks against changed files in each module matrix entry. Add the changed alarm-actions module to that matrix and remove unused AWS credentials and write permissions.
