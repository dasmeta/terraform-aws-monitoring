...
- Modify `.github/workflows/checkov.yaml`: call the official Checkov v12 action directly by immutable commit, with `soft_fail: true` to preserve the current advisory policy for scan findings. Scan repository-owned Terraform without recursively downloading third-party module trees in every matrix job. Do not use step-level `continue-on-error`, so action setup and scanner execution errors still fail the job.
